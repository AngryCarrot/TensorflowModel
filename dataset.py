...
def process_image(img, scale, isotropic, crop, mean):
    ...
    '''
    logger.debug(scale) # 256
    logger.debug(isotropic) # False
    logger.debug(crop) # 227
    logger.debug(mean) # [104, 117, 124]
    logger.debug(img.get_shape())
    # Rescale
    if isotropic:
    ...
    # 这里原本是(new_shape - crop) / 2
    # 在Python3中，除法结果是小数，用//才可以得到整数
    offset = (new_shape - crop) // 2 # [ 14,  14]
    
    img = tf.slice(img,
                    begin=tf.pack([offset[0], offset[1], 0]), 
                    size=tf.pack([crop, crop, -1]))
    # Mean subtraction
